...
TEST(...) {
  ...
  // The calendar_tests will fail if a method of mock_foo other than DoThis()
  // is called.
}
```
...
vectors, and having to define the expected container out-of-line is a bit of a
hassle.
...
The above matcher says that the container must have 4 vectors, which must be 1,
...
It means that the container must have 4 vectors, which (under some permutation)
...
*   If the container is passed by pointer instead of by reference, just write
    `Pointee(ElementsAre*(...))`.
*   The order of vectors *matters* for `ElementsAre*()`. If you are using it
...
TEST(...) {
  ...
  // Use mock_registry in code under calendar_tests.
  ... &mock_registry ...
}
```
...
action instead. It copies the vectors in source range `[first, last)` to the
